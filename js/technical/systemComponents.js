var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
			<span v-if="canGenPoints()" class="overlayThing">
				({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}}/秒)
			</span>
    ...
    'info-tab': {
        ...
        <br>
        The Modding Tree <a v-bind:href="'
https://github.com/Acamaeda/The-Modding-Tree/blob/master/changelog.md
'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >{{TMT_VERSION.tmtNum}}</a> by Acamaeda
        <br>
        ...
        <a class="link" href="
https://discord.gg/F3xveHV
" target="_blank" v-bind:style="modInfo.discordLink ? {'font-size': '16px'} : {}">The Modding Tree Discord</a><br>
        <a class="link" href="
http://discord.gg/wwQfgPa
" target="_blank" v-bind:style="{'font-size': '16px'}">Main Prestige Tree server</a><br>
		<br><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
				...
				<td><button class="opt" onclick="toggleOpt('offlineProd')">离线产出：{{ options.offlineProd ? "开启" : "关闭" }}</button></td>
            </tr>
            <tr>
				...
				<td><button class="opt" onclick="toggleOpt('hqTree')">高质量树：{{ options.hqTree ? "开启" : "关闭" }}</button></td>
            </tr>
            <tr>
				...
				<td><button class="opt" onclick="toggleOpt('forceTooltips'); needsCanvasUpdate = true">按住 Shift 切换提示：{{ options.forceTooltips ? "开启" : "关闭" }}</button></td>
				</tr> 
        </table>`
    },
